...
PRIVATE_KEY = os.getenv("BLIPHOOD_PRIVATE_KEY") or os.getenv("DEPLOYER_PRIVATE_KEY", "")
...
# -----------------------------------------------------------
def mint_bliphood(w3: Web3, contract: Contract, nonce: int, owner: str) -> Tuple[bool, str, int]:
    M = c("mint"); R = C.RST
    ...
    try:
        ...
        if receipt["status"] == 1:
            ...
            return True, tx_hash, gas_used
        else:
            print(f"{M}[Mint]{R} {c('fail')}[FAIL]{R} FAILED! Tx: {c('dim')}{tx_hash}{R}")
            return False, tx_hash, gas_used
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            success, tx_hash, gas_used = mint_bliphood(w3, contract, nonce, WALLET_ADDRESS)
